document.addEventListener('DOMContentLoaded', () => {
    ...
    if (pronunciationBtn) {
        pronunciationBtn.addEventListener('click', () => {
            const word = state.currentWords[state.currentWordIndex];
            if (word) {
                showVoicePracticeOverlay(word.english);
            }
        });
    }
    ...
    if (deepLearningBtn) {
        deepLearningBtn.addEventListener('click', () => {
            const word = state.currentWords[state.currentWordIndex];
            if (word) {
                showTranslatorOverlay(word.english);
            }
        });
    }
    ...
    // === 覆蓋層：發音練習與深入了解（沿用 1200 單字頁面模式） ===
    window.showVoicePracticeOverlay = function(word) {
    ...
    window.showTranslatorOverlay = function(word) {
        const overlay = document.createElement('div');
        overlay.className = 'practice-overlay';
        overlay.innerHTML = `
            <div class="practice-content">
    ...
    window.closePracticeOverlay = function() {
        ...
        if (data.status === 'completed') {
            ...
            contentDiv.innerHTML = `
                ...
                <div class="examples-section">
                    <h4>例句</h4>
                    <div class="examples-text">${data.examples}</div>
                </div>
            `;
        } else if (data.status === 'failed') {
            loadingSpinner.style.display = 'none';
            contentDiv.innerHTML = `<div class="error">翻譯失敗</div>`;
        } else {
