...
# 確保啟動時建立資料庫與必要欄位
try:
    # 確保 instance 目錄存在（SQLite 檔案路徑用到）
    ...
    with app.app_context():
        ...
        # 補齊舊資料庫可能缺的欄位：users.preferred_accent
        try:
            ...
            try:
                ...
                print(f"Failed to add preferred_accent column: {e}")
except Exception as e:
...
def voice_upload():
    ...
    try:
        ...
        if len(audio_data) < 5000:
            return jsonify({
                "success": False,
                "error": "錄音時間太短或上傳失敗，請重新錄音後再試"
            })

        print(f"[voice/upload] 音檔大小: {len(audio_data)} bytes")

        # 🔹 核心：語音轉文字（使用記憶體中的音檔）
        transcription_result = speech_to_text_from_memory(audio_data, audio.filename)
        ...
        # 音檔可播放 URL
        # served_filename = os.path.basename(target_path)
        return jsonify({
            "success": True,
            "reference": reference,
            ...
            "confidence": transcription_result.get('confidence', 0.9),
...
def evaluate_voice():
    try:
        ...
        transcribed = data.get("transcribed", "").strip().lower()

        if not reference or not transcribed:
            return jsonify({
                "success": False,
                "error": "缺少參考答案或辨識結果"
            }), 400
        ...
        try:
            import Levenshtein
            similarity = Levenshtein.ratio(reference, transcribed)
        except ImportError:
            similarity = 1.0 if reference == transcribed else 0.0
        ...
        if score == 100:
            ...
            feedback = "很好，接近正確。"
            suggestion = "再注意細節，例如母音或結尾音。"
        elif score >= 50:
            feedback = "有些正確，但需要加強。"
            suggestion = "多聽範例發音，練習語調。"
        else:
            feedback = "幾乎不正確，需要多加練習。"
            suggestion = "建議分音節慢慢練習，逐步改進。"
        ...
        return jsonify({
            ...
            "evaluation": {
                ...
                "improved_answer": reference  # 建議的正確說法
            }
        })

    except Exception as e:
        return jsonify({"success": False, "error": f"評估失敗: {str(e)}"}), 500
...
def process_speaking_audio():
    ...
    try:
        ...
        print(f"[speaking/process_audio] 音檔大小: {len(audio_data)} bytes")
        
        # 進行語音轉文字（使用記憶體版本）
        transcription_result = speech_to_text_from_memory(audio_data, audio_file.filename)
        
        if not transcription_result.get('success'):
            return jsonify({
                'success': False,
                'error': transcription_result.get('error', '語音辨識失敗')
            }), 400
...
def post_process_single_word(text, filename):
    """後處理單字辨識結果，嘗試從句子中提取單字"""
    if not text:
        return text
    
    # 常見的單字列表
    common_words = [
        "apple", "book", "cat", "dog", "hello", "yes", "no", "red", "blue", 
        "car", "house", "water", "food", "happy", "good", "bad", "big", 
        "small", "hot", "cold", "fast", "slow", "up", "down", "left", "right",
        "one", "two", "three", "four", "five", "six", "seven", "eight", "nine", "ten",
        "mother", "father", "sister", "brother", "friend", "teacher", "student",
        "school", "home", "park", "store", "hospital", "library", "restaurant"
    ]
    
    # 如果識別結果已經是單字，直接返回
    text_lower = text.lower().strip()
    if text_lower in common_words:
        print(f"🎯 直接識別為單字: {text_lower}")
        return text_lower
    
    # 如果識別結果是多個單字，嘗試找到最可能的單字
    words = text_lower.split()
    if len(words) > 1:
        # 檢查是否有常見單字在識別結果中
        for word in words:
            if word in common_words:
                print(f"🎯 從句子中提取單字: {word} (原句: {text})")
                return word
        
        # 如果沒有找到，檢查是否有部分匹配
        for word in words:
            for common_word in common_words:
                if word in common_word or common_word in word:
                    print(f"🎯 部分匹配提取單字: {common_word} (原句: {text})")
                    return common_word
    
    # 如果都找不到，返回原始結果
    print(f"⚠️ 無法提取單字，返回原始結果: {text}")
    return text

def speech_to_text_from_memory(audio_data, filename):
    """使用記憶體中的音檔進行語音轉文字"""
    print(f"🎤 開始使用 AssemblyAIManager 識別記憶體中的音檔: {filename}")
    
    try:
        # 創建臨時檔案用於 Assembly AI 處理
        import tempfile
        import os
        ...
        try:
            ...
            return {
                ...
                'words_count': len(processed_text.split()) if processed_text else 0,
                'original_text': transcript.text.strip()  # 保留原始識別結果
            }
...
def speech_to_text(audio_file_path):
    """使用新的 Assembly_api_manager 進行語音轉文字（保留原函數以向後兼容）"""
...
def fallback_speech_recognition_from_memory(audio_data, filename):
    """改進的本地語音識別方案（記憶體版本）"""
    try:
        import os
        import random
        ...
        return {
            ...
            'method': 'local_simulation_memory',
            'duration': round(duration, 2),
            'note': '本地語音處理 - 基於錄音特徵智能生成（記憶體版本）'
        }
